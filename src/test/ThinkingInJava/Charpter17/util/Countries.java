...
public class Countries {
    ...
    private static class FlyweightMap extends AbstractMap<String, String> {
        ...
        static class EntrySet extends AbstractSet<Map.Entry<String, String>> {
            private int size;
            //继承AbstractSet必须实现的方法，因为AbstractSet的顶级父接口未实现size()
            public int size() {
                return size;
            }
    ...
    static Map<String, String> select(final int size) {
        //返回一个重写entrySet()的FlyweightMap()
        return new FlyweightMap() {
            public Set<Map.Entry<String, String>> entrySet() {
                //返回长度为size 的 EntrySet
    ...
    // A partial list:
    public static Set<String> names(int size) {
        //keySet()在AbstractMap中被实现，返回key的Set集合
        return new HashSet<String>(select(size).keySet());//select(size)：一个具体长度的mao，
    }
