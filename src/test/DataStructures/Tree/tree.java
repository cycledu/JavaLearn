...
//		binaryTree.preOrder(); // 1,2,3,4
     ...
     * 以非递归的方式进行中序遍历
     */
    public static void inOrderNoRes(HeroNode root) {
        System.out.println("以非递归的方式中序遍历二叉树");
        Queue<HeroNode> list = new LinkedList<>();
        if (root == null) {
            ...
            while (root != null) {
                ...
                if (root.getLeft() != null) {
                    list.offer(root.getLeft());
                    root = root.getLeft();
                }
            }
            while (list.size() > 0) {
                HeroNode node = ((LinkedList<HeroNode>) list).getFirst();
