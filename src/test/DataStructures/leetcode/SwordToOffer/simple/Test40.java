...
public class Test40 {
    public int[] getLeastNumbers(int[] arr, int k) {
        for (int i = 0; i < arr.length; i++) {
            int temp = arr[i];
            int insertIndex = i - 1;
            while (insertIndex >= 0 && temp < arr[insertIndex]) { //如果后面的值小于前面的，则交换
    ...
    public int[] getLeastNumbers3(int[] arr, int k) {
        ...
        for (int num = 0; num < counter.length; num++) {
            //在这个有1001的位置的数组里，记录它的位置的树出现的次数，
            //总是前面的数小（因为题意是要找出出现的最小的k个数），所有只要前面的数出现了足够的次数，就不用统计后面的数了
    ...
    public int[] getLeastNumbers4(int[] arr, int k) {
        ...
        for (int num : arr) {
            ...
            if (entry.getKey() > num) {
                map.put(num, map.getOrDefault(num, 0) + 1);
                //前面放入一个新元素或更小元素的出现个数增加，后面就要将最大的元素直接去除或个数减1
                if (entry.getValue() == 1) {
                    map.pollLastEntry();
                } else {
