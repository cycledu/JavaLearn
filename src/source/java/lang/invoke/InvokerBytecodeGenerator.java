...
class InvokerBytecodeGenerator {
    ...
    void emitStaticInvoke(MemberName member, Name name) {
        ...
        if (member.getDeclaringClass().isInterface() && refKind == REF_invokeVirtual) {
            // Methods from Object declared in an interface can be resolved by java8.JVM to invokevirtual kind.
    ...
    private void emitReturn(Name onStack) {
        ...
        if (rtype == V_TYPE) {
            ...
            // it doesn't matter what rclass is; the java8.JVM will discard any value
        } else {
