...
class Invokers {
    ...
    /*non-public*/ static MemberName invokeBasicMethod(MethodType basicType) {
        ...
        try {
            ...
            throw newInternalError("java8.JVM cannot find invoker for "+basicType, ex);
        }
    }
    ...
    private static LambdaForm invokeHandleForm(MethodType mtype, boolean customized, int which) {
        ...
        if (MTYPE_ARG >= INARG_LIMIT) {
            ...
            // else if isLinker, then MTYPE is passed in from the caller (e.g., the java8.JVM)
        }
        ...
        if (isLinker)
            lform.compileToBytecode();  // java8.JVM needs a real methodOop
        if (isCached)
    ...
    WrongMethodTypeException newWrongMethodTypeException(MethodType actual, MethodType expected) {
        // FIXME: merge with java8.JVM logic for throwing WMTE
